...
def build_link_index():
    index = dict()
    for link in links:
        index[link.id] = link_by_id(link.id)
    return index
...
# using 'get' method will return None if invalid input rather than crash/error
def link_by_id(link_id):
